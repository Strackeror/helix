...
impl FileBlame {
    ...
    pub fn blame_for_line(&self, line: u32, inserted_lines: u32, removed_lines: u32) -> LineBlame {
        ...
        let line_blame = LineBlame {
            ...
            commit_date: time
                .map(|time| time.format(gix::date::time::format::SHORT))
                .and_then(|r| r.ok()),
    ...
    pub fn try_new(file: PathBuf, trust_full: bool) -> Result<Self> {
        let repo = get_repo_dir(&file)?;
        let thread_safe_repo = open_repo(repo, trust_full).context("Failed to open git repo")?;
